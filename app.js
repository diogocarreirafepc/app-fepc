...
const DEFAULT = {baseRate:9.54, suppRate:13.65, baseMonthlySalary:1446.51, fixedSupplements:623.72, hoursPerWeekday:7, adseRate:3.5, ssRate:11, irsRate:13.53, unionRate:0.65};
...
function loadSettingsInputs(){
  const vals={baseMonthlySalaryInput:settings.baseMonthlySalary,fixedSupplementsInput:settings.fixedSupplements,baseRateInput:settings.baseRate,suppRateInput:settings.suppRate,adseInput:settings.adseRate,ssInput:settings.ssRate,irsInput:settings.irsRate,unionInput:settings.unionRate,holidaysInput:monthSettings.holidays||0,vacationDaysInput:monthSettings.vacationDays??1};
  for(const id in vals){let el=document.getElementById(id);if(el)el.value=vals[id]}
}

function saveSettings(){
  settings={baseMonthlySalary:+(document.getElementById("baseMonthlySalaryInput").value||1446.51),fixedSupplements:+(document.getElementById("fixedSupplementsInput").value||623.72),baseRate:+(document.getElementById("baseRateInput").value||9.54),suppRate:+(document.getElementById("suppRateInput").value||13.65),hoursPerWeekday:7,adseRate:+(document.getElementById("adseInput").value||3.5),ssRate:+(document.getElementById("ssInput").value||11),irsRate:+(document.getElementById("irsInput").value||13.53),unionRate:+(document.getElementById("unionInput").value||0.65)};
...
function calculate(){
  ...
  let baseSalary=+settings.baseMonthlySalary||0; let fixedSupplements=+settings.fixedSupplements||0;
  ...
  const map={totalHours:h(totalWorked),targetHours:h(needed),nightHours:h(night),extra125:h(e125),extra1375:h(e1375),holiday150:h(e150),vacationDaysCount:vacationDaysInMonth(),holidayDaysCount:holidayDaysInMonth(),compDaysCount:compDaysInMonth(),bankHours:(bank>=0?"+":"")+h(bank),fixedBaseSalary:euro(baseSalary),fixedSupplementsValue:euro(fixedSupplements),fixedGrossTotal:euro(fixedGross),overtime125Detail:`${h(e125)} · ${euro(overtime125Value)}`,overtime1375Detail:`${h(e1375)} · ${euro(overtime1375Value)}`,overtime150Detail:`${h(e150)} · ${euro(overtime150Value)}`,overtimeTotalValue:euro(overtimeTotal),adse:euro(adse),ss:euro(ss),irs:euro(irs),sindicato:euro(sindicato),totalDesc:euro(totalDesc),resultFixedGross:euro(fixedGross),resultOvertimeGross:euro(overtimeTotal),totalGross:euro(gross),resultDiscounts:euro(totalDesc),netPay:euro(net),grossDebt:euro(paidGross>0?gross-paidGross:0),netDebt:euro(paidNet>0?net-paidNet:0),missing125:h(Math.max(0,e125-(receiptValues.paidExtra125||0))),missing1375:h(Math.max(0,e1375-(receiptValues.paidExtra1375||0))),missing150:h(Math.max(0,e150-(receiptValues.paidExtra150||0)))};
